...
class ResultSurvivalScene(GameScene):
    ...
    def change_locale(self):
        ...
        self.ui["condition"].set_text(f"{self.parent.t('txt_enemy')} {SurvivalRule.FORCE_LIST[self.parent.states.survival_enemies]}")
    ...
    def setup_listdata(self):
        ...
        data = self.parent.data_man.get_survival_cleardata(self.parent.states.survival_enemies)
        self.basedata = data
        tmpy = 0
        #print(data)
    ...
    def draw_resultlist(self):
        if not self.states["is_drawend"]:
            ...
            if self.states["detailpage"] == 0:
                ...
                pyxel.text(pos(1),pos(10),f"Mode: {self.parent.t('txt_survival')}", pyxel.COLOR_WHITE, font=self.parent.jp_fontmisaki)
                pyxel.text(pos(1),pos(11),f"{self.parent.t('txt_enemy2')} {SurvivalRule.FORCE_LIST[self.parent.states.survival_enemies]}", pyxel.COLOR_WHITE, font=self.parent.jp_fontmisaki)
                pyxel.text(pos(1),pos(12),f"{self.parent.t('txt_wave')} {self.select_data['wave']}", pyxel.COLOR_WHITE, font=self.parent.jp_fontmisaki)
                ...
                if self.parent.states.current_gamemode == GameMode.TYPE_SURVIVAL:
                    pyxel.text(pos(2), pos(2), f"Time:{self.select_data['play_time']}", pyxel.COLOR_WHITE, font=self.parent.jp_fontmisaki)
                    pyxel.text(pos(2), pos(3), f"{self.parent.t('txt_wave')} {self.select_data['wave']}", pyxel.COLOR_WHITE, font=self.parent.jp_fontmisaki)
